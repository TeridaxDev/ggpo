...
public:
	...
	*/
	virtual int RecvFrom(char* buffer, char** storage, int len, int flags, int* connection_id) = 0;
...
class SteamConnection : public ConnectionManager
{
public:
	SteamConnection();
	virtual ~SteamConnection();

	virtual void GivePointers(int (*SendDel) (char* buffer, int len, int flags, int connection_id), 
		int (*RecvDel) (char** buffer, int len, int flags, int* connection_id));

	virtual int SendTo(char* buffer, int len, int flags, int connection_id);

	virtual int RecvFrom(char* buffer, char** storage, int len, int flags, int* connection_id);

	virtual int AddConnection(int steamID);

protected:

	virtual std::shared_ptr<ConnectionInfo> BuildConnectionInfo(int steamID);

	int (* SendToDel) (char* buffer, int len, int flags, int connection_id);
	int (* RecvFromDel) (char** buffer, int len, int flags, int* connection_id);

};

class SteamInfo : public ConnectionInfo
{
public:
	SteamInfo(int steamID);

	unsigned long steamUserID;
