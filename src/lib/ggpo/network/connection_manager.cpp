...
SteamInfo::SteamInfo(int steamID)
{
	steamUserID = steamID;
}

std::string SteamInfo::ToString()
{
	return std::to_string(steamUserID);
}
...
void SteamConnection::GivePointers(int(*SendDel)(char* buffer, int len, int flags, int connection_id), int(*RecvDel)(char** buffer, int len, int flags, int* connection_id))
{
	SendToDel = SendDel;
	RecvFromDel = RecvDel;
}

int SteamConnection::SendTo(char* buffer, int len, int flags, int connection_id)
{
	return SendToDel(buffer, len, flags, connection_id);
}

int SteamConnection::RecvFrom(char* buffer, char** storage, int len, int flags, int* connection_id)
{
	int done = RecvFromDel(&buffer, len, flags, connection_id);
	if (done == -1)
		return done;
...
int SteamConnection::AddConnection(int steamID)
{
	_connection_map.insert(std::make_pair(_id_to_issue, BuildConnectionInfo(steamID)));
	return _id_to_issue++;
}


std::shared_ptr<ConnectionInfo> SteamConnection::BuildConnectionInfo(int steamID)
{
	return std::static_pointer_cast<ConnectionInfo>(std::make_shared<SteamInfo>(steamID));
}
...
std::string UPDInfo::ToString() {
	char dst_ip[1024];
	char buffer[100];
	sprintf_s(buffer, "Connection: IP: %s, Port: %d",
