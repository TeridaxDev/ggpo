...
Udp::OnLoopPoll(void* cookie)
{
	...
	char* storage = &recv_buf[0];// (char*)malloc(MAX_UDP_PACKET_SIZE);

	for (;;) {
		int connection_id = -1;
		int len = _connection_manager->RecvFrom((char*)recv_buf, &storage, MAX_UDP_PACKET_SIZE, 0, &connection_id);
